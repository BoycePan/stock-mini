 ...
 * 且**双指捏合一律以可见窗口最右侧那根 K 线为基准**（两指落点只决定指距，不决定锚点）。
 ...
 * ```
 * 缩放换算（上下限夹紧）复用 utils/kline-viewport.ts 的 scaleViewport，
 * 锚点固定为可见窗口**最右侧那根 K 线**（两指落点不参与锚点换算）。
 */

import { MIN_VIEW_BARS, scaleViewport, type ViewportState } from './kline-viewport'
...
export function moveGesture(
  ...
  if (state.mode === 'pinch' || touches.length >= 2) {
    ...
    // 锚点固定为窗口最右侧那根：只按指距比例改根数，两指落点不参与换算
    const viewport = scaleViewport(config.total, state.base, dist / state.pinchDist)
