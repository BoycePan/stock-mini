 ...
 *
 * formatChange 会把 |change| 小于 0.005 的值四舍五入显示成「0%」（数值上等于 0 的结果已由
 * formatChange 统一归一为不带符号的「0%」，不再输出「+0% / -0%」）；若仍按原始数值符号着色，
 * 就会出现「文本明明是 0%，却按涨/跌标成红/绿、还带箭头」的自相矛盾（例如 change = 0.004 → 「0%」却标红）。
 *
 * 这里仍一律以「展示是否为 0%」为准（对上述归一化的兜底判定）：显示为 0% 时按平盘（flat）处理，
 * 并把文本归一化为「0%」，避免表格里出现「+0% / -0%」这类易被误读的写法。
 */
