 ...
 * - 连续交易标的（金银现货 XAUUSD/XAGUSD、COMEX 金属、沪期货夜盘、外汇、美元指数、原油等）没有「时间还没到」的概念，
 *   维持原有拉伸绘制（continuous）。
 *
...
// ---------------------------------------------------------------------------
// 五日分时（多日分钟线）：不按当日时段铺点，改为按自然日分段 + 等分拉伸
// ---------------------------------------------------------------------------

/** 五日分时的一段（同一自然日的连续数据点区间，[start, end] 为闭区间下标） */
export interface MinuteDaySlice {
  /** 自然日 "YYYY-MM-DD" */
  date: string
  start: number
  end: number
...
export function splitMinuteDays(
  ...
  const slices: MinuteDaySlice[] = []
  for (let i = 0; i < points.length; i += 1) {
    const date = minutePointDate(points[i])
    if (!date) return []
    const last = slices[slices.length - 1]
    if (last && last.date === date) {
      last.end = i
    } else {
      slices.push({ date, start: i, end: i })
    }
  }
  return slices
}

/** 取数据点的日期 "YYYY-MM-DD"（timeFull 优先，其次 time 自带日期前缀），无则 null */
export function minutePointDate(point?: { time: string; timeFull?: string }): string | null {
  if (!point) return null
  const full = point.timeFull ?? point.time ?? ''
  const match = /^(\d{4}-\d{2}-\d{2})/.exec(full)
  return match ? match[1]! : null
}

/** 五日分时横轴日期标签："2026-09-09" → "09-09" */
export function minuteDayLabel(date: string): string {
