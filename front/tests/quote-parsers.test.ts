...
test('新浪 gb_ 美股（宏观消费方）：现价 [1]、涨跌幅 [2]、涨跌额 [4]、昨收 [26]', () => {
  // 与下方盘前用例同源的实测 36 字段夹具（gb_nvda）：[1]现价 213.05 / [2]涨跌幅 2.19 /
  // [4]涨跌额 4.57 / [26]昨收 208.48。曾按 [2] 取昨收、[3] 取涨跌幅（[3] 实为时间串，
  // 会落到 lastEightDigitsPct 兜底取到末尾常量）导致 TLT 卡片昨收/涨跌额/涨跌幅全错。
  const quote = parseSinaQuote('gb_TLT', SINA_GB_FIELDS)
  assert.equal(quote.price, 213.05)
  assert.equal(quote.changePercent, 2.19)
  assert.equal(quote.change, 4.57)
  assert.equal(quote.previousClose, 208.48)
  // 涨跌额缺失（空串）时按 现价-昨收 反推；涨跌幅仍取 [2]
  const noChange = [...SINA_GB_FIELDS]
  noChange[4] = ''
  const quote2 = parseSinaQuote('gb_TLT', noChange)
  assert.ok(Math.abs((quote2.change as number) - 4.57) < 0.001)
  assert.equal(quote2.changePercent, 2.19)
})
 ...
 * gb_ 布局消费方较多（盘中字段 / 盘前字段 / 行情时间），共用同一份夹具避免副本走样：
 * [1] 现价 / [2] 盘中涨跌幅% / [3] 行情时间（北京时间）/ [4] 涨跌额 / [26] 昨收。
 */
const SINA_GB_FIELDS = [
  '英伟达', // [0] 名称
