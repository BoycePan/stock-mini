 ...
 * 入口卡「全部板块」。全部板块均保持「A股 BK 板块 + 美股代理股」双源口径，
...
export const INDUSTRY_BOARDS: IndustryBoardConfig[] = [
  ...
  // 稀土：原 BK1016 已被东财现行口径改指「汽车服务」（代码漂移），2026-09-07 改为现行
  // 行业清单实测的 BK1626「稀土」（东财 searchapi/ulist 实测），展示名与代理股不变
  ...
  // 钢铁/物流/航空/房地产）移除，A 股全行业覆盖走「全部板块」入口卡。
