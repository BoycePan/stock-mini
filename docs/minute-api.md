...
>
> 该页面后续扩展为**六周期图表**（分时 / 五日 / 日K / 周K / 月K / 年K + 成交量 + MACD），
> 另见 [`行情页多周期图表.md`](./行情页多周期图表.md)：本文档描述的分时链路（东财 trends2 / 腾讯 / Yahoo）
> 与映射表未变，图表组件已由 `components/minute-chart` 换为 `packageQuote/components/quote-chart`（分时模式）。
...
- 已有实测：24 个板块的 `us-BKxxxx` 全部可命中（`scripts/verify-minute.ts` 全量 0 失败）；2026-09-07 扩充的 15 个板块走同一合成逻辑，新代理 secid 均经东财 ulist/searchapi 核实。
